...
public partial class TabContentView : ReactiveUserControl<TabContentViewModel>
{
    ...
    public TabContentView()
    {
        ...
        _webBrowser.RequestContext = new RequestContext();
    }
    ...
    {
        if (Keyboard.Modifiers is ModifierKeys.Alt && e.SystemKey == Key.Left &&
            _webBrowser.BackCommand.CanExecute(null))
        {
            _webBrowser.BackCommand.Execute(null);
            e.Handled = true;
        }
        else if (Keyboard.Modifiers is ModifierKeys.Alt && e.SystemKey == Key.Right &&
                 _webBrowser.ForwardCommand.CanExecute(null))
        {
            _webBrowser.ForwardCommand.Execute(null);
            e.Handled = true;
        }
        else if (Keyboard.Modifiers is ModifierKeys.Control && e.Key == Key.R &&
                 _webBrowser.ReloadCommand.CanExecute(null))
        {
